...
export function CarePlans() {
  ...
  return (
    <View style={styles.wrap}>
            ...
            {on ? (
              <Appear>
                <View style={styles.progressRow}>
                  ...
                  <Text
                    style={styles.count}
                    accessibilityLabel={`${n} of ${plan.sessions} booked this year`}
                  >
                    {n}/{plan.sessions}
                  </Text>
                </View>
                {covered.map(({ c }) => {
                  const back = rebate(
                    c.practical.fee,
                    c.practical.gapAfterMedicare,
                  );
                  return (
                    <View key={c.id} style={styles.row}>
                      <Text style={styles.name}>{c.firstName}</Text>
                      <Text style={[styles.value, !back && styles.muted]}>
                        {back ? `$${back} back` : "Rebate: ask"}
                      </Text>
                    </View>
                  );
                })}
      ...
      {/* Only once someone's on your team (My care handles an empty team). */}
      {team.length > 0 ? (
            ...
            return (
              ...
              >
                ...
                <View style={styles.when}>
                  ...
                  <Text style={[styles.whenValue, styles.next]}>
                    {next ? short(next) : "—"}
                  </Text>
                </View>
              </PressScale>
