...
| **7. Hardening** 🟡 built, awaiting sign-off | Accessibility pass, performance budget, privacy review, safety copy reviewed by a clinical advisor. | WCAG AA checks pass; load under 2 s; the checklist in §9 is signed off. Ready for moderated user testing. **Status:** WCAG AA contrast, headings, roles and 44 px targets fixed; JS budget in CI (395 KB gz of 450); randomised engine tests; [`docs/privacy.md`](privacy.md) and [`docs/safety.md`](safety.md) written. Waiting on a clinical advisor (safety copy and urgent list) and the privacy decisions listed there. |
| **8. Claude agent + API (final stage)** 🟡 extraction built | Vercel Functions `/api/turn`, `/api/matches`, `/api/feedback`; Claude extraction with a JSON schema; safety classifier; SPA rewrite excluding `/api`. Client switches from fixtures to the API. | Real free text produces sensible follow-ups and matches; p75 latency under 3 s; the extraction eval set (§13) meets its threshold; no medical advice in 50 red-team prompts. **Status:** `/api/extract` is live and falls back to keywords until `ANTHROPIC_API_KEY` is set. Assistant replies are templated from the signal diff; medical questions get a fixed no-advice reply, checked against 50 red-team prompts (`evals/redteam.json`). Rerun that set if Claude ever writes replies. See Phase 8 notes. |
...
### Phase 9 notes (discovery and care team, 26 Sep)

The direction is set out in [`docs/VISION.md`](VISION.md): concierge care, for everyone.

- **Discovery:**
  - a grid across 7 professions (26 real professionals);
  - Explore, a swipeable card per profession;
  - "Also could help" after results.
- **Care:**
  - goals in Profile, which also tilt searches at low confidence;
  - My care: your team, empty slots from goals, and booking reminders. On phones these go to the device calendar; on web to Google Calendar or an .ics file;
  - starting a search from your team or discovery pre-fills your goals.
- **Supply:** Join WATL, a professional submits their facts in their own words, which are queued for review; then `interview.py pull`.
- **Feel:**
  - swipe to pass or save, a filled heart with a pop and ring, and progress dots;
  - haptics on phones;
  - the back button everywhere, and less text.
- **Next:**
  1. email a professional when they're live;
